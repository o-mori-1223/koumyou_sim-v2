"""gsolver_reference_2col_51_81.py の構造について:
  1. Gsolverインポート用テキストを data/gsolver_import_2col_51_81.txt に書き出す
  2. rcwa_mh でのスペクトル(法線入射, s/p, 380-780nm 10nm刻み, NORDER=43)を計算し
     data/rcwa_mh_2col_51_81_result.json に保存する（Gsolverの結果が揃ったら比較用）

Gsolver側の手順: Parameters: Units=Microns, Period=1.2。ORDERS=41以上に設定。
File > Import Text で data/gsolver_import_2col_51_81.txt を読み込み（Approximateは
...
from gsolver_reference_2col_51_81 import build_layers, PITCH_NM, NK_FN_MAP, NK_SUBST_FN
...
out_txt = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'gsolver_import_2col_51_81.txt')
...
out_json = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'rcwa_mh_2col_51_81_result.json')
