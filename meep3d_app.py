...
表示している色・スペクトルは「反射率」ではなく、円盤という摂動が全方向
（上・側方・基板透過分の変化）に再配分した散乱効率 Q_scatter,up
（1.0でクリップ）である（詳細はMEEP3D/geometry.py, HANDOVER.md参照）。
"""
import json
import os
...
st.caption(
    ...
    "周期構造の反射率ではなく、円盤という摂動が全方向（上・側方・基板透過分の"
    "変化）に再配分する散乱効率 Q_scatter,up（1.0でクリップ）です。上から見た"
    "色の予測用途では基板透過分の変化も含む分だけ過大評価になりうる点に注意"
    "してください。角度掃引(30°/60°)はPhase 2で対応予定、未実装です。"
)
